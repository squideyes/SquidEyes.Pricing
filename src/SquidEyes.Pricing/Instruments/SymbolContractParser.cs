...
public static class SymbolContractParser
{
    ...
    {
        if (string.IsNullOrEmpty(symbolSegment))
            return null;

        if (symbolSegment.Contains('.'))
            return null;

        int i = symbolSegment.Length - 1;

        while (i >= 0 && char.IsDigit(symbolSegment[i]))
            i--;

        if (i < 1 || !char.IsLetter(symbolSegment[i]))
            return null;

        var yearPart = symbolSegment[(i + 1)..];

        if (yearPart.Length is < 1 or > 2)
            return null;

        if (!int.TryParse(yearPart, out var yearVal))
            return null;
        ...
        if (!Instrument.IsSupported(symbolPart))
            return null;

        var symbol = Instrument.Parse(symbolPart).Symbol;
